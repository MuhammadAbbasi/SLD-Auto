# -*- coding: utf-8 -*-
"""
sld_gui_v7.py  -  DC Single Line Diagram Generator  v7
=======================================================
Fixes vs v6:
  - Panel diagram cells (numbered boxes) now regenerated from actual pps setting.
  - Overlapping text prevention: panel cells and CC specs de-conflicted by Y proximity.
  - New option: "Print String Names" — show or hide the "String X.Y.Z" prefix in labels.

GUI:  python sld_gui_v7.py
CLI:  python sld_gui_v7.py --excel <excel_path> [options]
"""
...
# Y-band default in the template DXF that contains Inverter 1.1
_TMPL_Y_MIN = 159_400
_TMPL_Y_MAX = 168_000
...
# ─────────────────────────────────────────────────────────────────────────────
#  PANEL DIAGRAM HELPERS (v7)
# ─────────────────────────────────────────────────────────────────────────────

def _detect_panel_cells(tmpl, panel_count_y, y_tol=600):
    """
    Find panel number MTEXT (classified as 'panel_number') and the small
    LWPOLYLINE cells that form the panel-diagram boxes in the template.
    Returns (number_dicts, rect_dicts) both sorted by ascending X.
    """
    nums  = [d for d in tmpl
             if d['type'] == 'MTEXT' and d.get('cls') == 'panel_number'
             and abs(d['y'] - panel_count_y) < y_tol]
    rects = []
    for d in tmpl:
        if d['type'] != 'LWPOLYLINE' or not d.get('closed', False):
            continue
        pts = d.get('pts', [])
        if len(pts) < 4:
            continue
        ...
        if abs(mid_y - panel_count_y) < y_tol and 20 < w < 500 and 10 < h < 200:
            rects.append(d)
    nums.sort(key=lambda d: d['x'])
    rects.sort(key=lambda d: min(p[0] for p in d['pts']))
    return nums, rects


def _stamp_panel_diagram(msp, num_dicts, rect_dicts, pps, dx, dy, log):
    """
    Regenerate the panel numbered-cell diagram for the actual pps count.
    Clones the first template cell and steps it pps times to the right.
    """
    if pps <= 0:
        return
    ...
    if num_dicts:
        ref = num_dicts[0]
        if len(num_dicts) >= 2:
            step = num_dicts[1]['x'] - num_dicts[0]['x']
        elif rect_dicts and len(rect_dicts) >= 2:
            step = (min(p[0] for p in rect_dicts[1]['pts'])
                    - min(p[0] for p in rect_dicts[0]['pts']))
        else:
            xs = [p[0] for p in rect_dicts[0]['pts']] if rect_dicts else []
            step = (max(xs) - min(xs)) if xs else 100
        for i in range(pps):
            d_copy = dict(ref, x=ref['x'] + i * step)
            _place_mtext(msp, d_copy, dx, dy, str(i + 1), log)

    # ── cell rectangles ──────────────────────────────────────────────────────
    if rect_dicts:
        ref = rect_dicts[0]
        if len(rect_dicts) >= 2:
            x0 = min(p[0] for p in rect_dicts[0]['pts'])
            x1 = min(p[0] for p in rect_dicts[1]['pts'])
            step = x1 - x0
        elif num_dicts and len(num_dicts) >= 2:
            step = num_dicts[1]['x'] - num_dicts[0]['x']
        else:
            xs = [p[0] for p in ref['pts']]
            step = max(xs) - min(xs)
        x_base = min(p[0] for p in ref['pts'])
        for i in range(pps):
            shift = i * step
            new_pts = [[p[0] + shift, p[1]] for p in ref['pts']]
            d_copy = dict(ref, pts=new_pts)
            _place_entity(msp, d_copy, dx, dy, log)
...
def _generate(cfg, log):
    ...
    show_string_name  = cfg.get('show_string_name', True)   # v7: toggle string name prefix
    ...
    # ── 3b. Detect panel diagram cells (v7) ───────────────────────────────────
    panel_count_d = next((d for d in tmpl_texts if d.get('cls') == 'panel_count'), None)
    panel_count_y = panel_count_d['y'] if panel_count_d else None
    _panel_nums  = []   # list of MTEXT dicts (individual numbers)
    _panel_rects = []   # list of LWPOLY dicts (cell boxes)

    if panel_count_y is not None:
        _Y_TOL = 600
        # Re-classify small integer MTEXT near the panel_count Y as 'panel_number'
        for d in tmpl_texts:
            if d.get('cls') == 'fixed':
                txt = _strip_mtext_fmt(d['text']).strip()
                if re.match(r'^\d{1,2}$', txt) and 1 <= int(txt) <= 50:
                    if abs(d['y'] - panel_count_y) < _Y_TOL:
                        d['cls'] = 'panel_number'
        # Collect classified panel numbers
        _panel_nums = [d for d in tmpl_texts if d.get('cls') == 'panel_number']
        # Find cell LWPOLYLINE rectangles near panel_count_y
        for d in tmpl:
            if d['type'] == 'LWPOLYLINE' and d.get('closed', False):
                pts = d.get('pts', [])
                if len(pts) >= 4:
                    xs = [p[0] for p in pts]; ys = [p[1] for p in pts]
                    w  = max(xs) - min(xs);   h  = max(ys) - min(ys)
                    mid_y = (max(ys) + min(ys)) / 2
                    if abs(mid_y - panel_count_y) < _Y_TOL and 20 < w < 500 and 10 < h < 200:
                        d['_panel_cell'] = True
                        _panel_rects.append(d)
        log(f"Panel diagram: detected {len(_panel_nums)} number labels, {len(_panel_rects)} cells "
            f"(template pps ≈ {max((int(_strip_mtext_fmt(d['text']).strip()) for d in _panel_nums), default=0)})")
    ...
    def make_string_label(T, I, mppt, port):
        ...
        if port - 1 < len(lst):
            ...
            if panels_per_string > 0:
                panel_part = (f"{panels_per_string}x {panel_model}{suffix}"
                              if panel_model else f"{panels_per_string}P{suffix}")
            else:
                panel_part = ""
            ...
            if show_string_name:
                ...
                # v7: hide "String X.Y.Z" prefix — show only panel info
                label = panel_part if panel_part else f"String {name}"
    ...
    for idx, (T, I) in enumerate(inv_list):
        ...
        for tmpl_idx, d in enumerate(tmpl):
            # v7: skip panel diagram cells — regenerated correctly below
            if d.get('_panel_cell'):
                continue
            if d['type'] == 'MTEXT' and d.get('cls') == 'panel_number':
                continue
        ...
        # v7: Stamp panel diagram cells regenerated for actual pps
        if panels_per_string > 0 and (_panel_nums or _panel_rects):
            _stamp_panel_diagram(msp, _panel_nums, _panel_rects,
                                 panels_per_string, dx, dy, log)
        ...
        if missing and proto_ents:
            for miss_m in missing:
                ...
                for d in proto_ents:
                    ...
                    _place_entity(msp, d_copy, dx, dy + shift_y, log)
...
class SLDAppV7(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("A176LAB - DC Single Line Diagram Generator v7")
    ...
    def _add_header(self):
        ...
        # Load Tk logo from file path
        logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'logoA176LAB.jpg')
        logo = None
        try:
        ...
        ttk.Label(txt, text="DC Single Line Diagram Generator v7",
                  font=('Segoe UI', 14, 'bold')).pack(anchor='w')
        ttk.Label(txt, text="A176 LAB  –  Think different project (Template-Based CAD Generation)",
    ...
    def _build_files_tab(self, nb):
        ...
        default_tmpl = _DEFAULT_TEMPLATE
        ...
        hint = (
            "Template DXF      - Source DXF containing Inverter 1.1 template geometry slice.\n"
    ...
    def _build_equip_tab(self, nb):
        ...
        self.f_show_str_name = _CheckboxRow(
            left_i, "Print String Names:", default=True)
        self.f_show_str_name.pack(fill='x', pady=2)
        ttk.Label(left_i,
                  text="  → When unchecked, labels show only panel count and model (no 'String X.Y.Z' prefix).",
                  foreground='gray', font=('Segoe UI', 8, 'italic')
                  ).pack(anchor='w', padx=(190, 6), pady=(0, 4))
    ...
    def _collect(self):
        return {
            ...
            'show_cable_info':   self.f_show_cable_info.get(),
            'show_string_name':  self.f_show_str_name.get(),
        }
    ...
    def _on_generate(self):
        ...
        self._log("=" * 64)
        self._log("Starting template-based SLD diagram generation (v6) ...")
...
def _build_cli():
    p = argparse.ArgumentParser(
        prog='sld_gui_v7.py',
        description='DC Single Line Diagram Generator v7 (Template-Based)',
    )
    p.add_argument('--template',        metavar='PATH', default=_DEFAULT_TEMPLATE)
    p.add_argument('--excel',           metavar='PATH')
    p.add_argument('--output',          metavar='PATH')
    p.add_argument('--panel-model',     metavar='NAME',  default='')
    p.add_argument('--panels',          metavar='N',     default='20')
    p.add_argument('--inv-model',       metavar='NAME',  default='')
    p.add_argument('--max-mppts',       metavar='N',     default='Auto')
    p.add_argument('--dc-power',        metavar='N',     default='350')
    p.add_argument('--ac-power',        metavar='N',     default='320')
    p.add_argument('--temp',            metavar='N',     default='40')
    p.add_argument('--tx-power',        metavar='NAME',  default='')
    p.add_argument('--show-cable',      action='store_true', help='Show cable length/section details')
    p.add_argument('--no-string-names', action='store_true', help='Omit String X.Y.Z prefix from labels')
    p.add_argument('--col-spacing',     metavar='N',     default=str(_COL_SPACING_DEFAULT))
    p.add_argument('--row-spacing',     metavar='N',     default=str(_ROW_SPACING_DEFAULT))
    p.add_argument('--circle-radius',   metavar='N',     default='24.59')
    p.add_argument('--text-size',       metavar='N',     default='60.44')
    p.add_argument('--heavy-section',   metavar='SEC',   default='1x10')
    p.add_argument('--heavy-linetype',  metavar='NAME',  default='TRATTEGGIATA')
    p.add_argument('--heavy-color',     metavar='ACI',   default='40')
    p.add_argument('--heavy-layer',     metavar='NAME',  default='TRATTEGGIATA')
    return p
...
def _run_cli(args):
    if not args.excel:
        ...
        sys.exit(1)
    if not args.output:
        args.output = os.path.splitext(args.excel)[0] + '_SLD_Generated.dxf'
    ...
    cfg = {
        ...
        'show_string_name':  not args.no_string_names,
