...
#  All values tied to the standard template DXF geometry.
# ─────────────────────────────────────────────────────────────────────────────
...
def _generate(cfg, log):
    ...
    if _SHEET not in wb.sheetnames:
        raise RuntimeError(
            f"Sheet '{_SHEET}' not found in the workbook.\n"
    ...
    # Strip formatting before matching so "{\fSwis721 BT|...;1-1}" is found.
    ...
    for idx, (T, I) in enumerate(inv_list):
        ...
        for (mppt, port), sl in mppt_map.items():
            ...
            _place_mtext(msp, sl_wide, dx, dy, make_string_label(T, I, mppt, port), log)
